...
public class GraphPanel extends JPanel implements Observer {
	...
	public void drawGeneratedPolygon(final Graphics graphic) {
		java.util.List<Point2D.Double> pointList = polygonModel.getPoly()
				.getPointList();
		int points = polygonModel.getPoly().getNumPoints();
		changeColor(graphic);
		for (int i = 0; i < points; i++) {
			graphic.drawLine(
					(int) (pointList.get(i).getX() * (width / 10)),
		...
		for (Point2D.Double p : pointList) {
			graphic.fillOval((int) (p.getX() * (width / 10)) - 2,
					(int) (p.getY() * (height / 10)) - 2, 4, 4);
		}
	}

	public void drawGuessed(final Graphics graphic) {
		java.util.List<Point2D.Double> incorrectPoints = polygonModel
				.getIncorrect();
		java.util.List<Point2D.Double> correctPoints = polygonModel
				.getCorrect();
		graphic.setColor(new Color(0, 152, 255));
		for (Point2D.Double correct : correctPoints) {
			graphic.fillOval((int) (correct.getX() * (width / 10)) - 2,
					(int) (correct.getY() * (height / 10)) - 2, 4, 4);
		}

		graphic.setColor(new Color(229, 29, 23));
		for (Point2D.Double incorrect : incorrectPoints) {
			graphic.fillOval((int) (incorrect.getX() * (width / 10)) - 2,
					(int) (incorrect.getY() * (height / 10)) - 2, 4, 4);
		}
	}
