...
public class RunController implements ActionListener {
	...
	private void runClass() throws SecurityException {
		try {
			...
			areaLoader = CheckAndCompileFile.compileFileAndReturnLoader(pModel,
					stuFile);
			areaLoader = new StudentsClassLoader(file, pModel);
			PolygonAreaIF polygonArea = (PolygonAreaIF) areaLoader
					.getInstance();
			double area = polygonArea.estimate(pModel.getMaxGuesses());
			pModel.scoreGuessedArea(area);
		} catch (ClassFormatError e) {
			dialogueBox(
					"The class is formatted incorrectly!/n Your file could not be compiled.",
					"Class Format Error!");
		} catch (MalformedURLException e) {
			dialogueBox("The file path is incorrectly formatted!",
					"Malformed URL!");
		} catch (ClassNotFoundException e) {
			dialogueBox("Your class must be named PolygonArea",
					"Class Not Found!");
		} catch (IllegalArgumentException e) {
			dialogueBox("Make sure your method takes in only an integer!",
					"Incorrect Arguments");
		} catch (InstantiationException e) {
			dialogueBox("Test Instantiation.", "Error!");
		} catch (IllegalAccessException e) {
			dialogueBox("Make sure your estimate method is public",
					"Estimate not accessable");
		} catch (InvocationTargetException e) {
			dialogueBox(e.getMessage(), "Your code has thrown an error.");
		} catch (NoSuchMethodException e) {
			dialogueBox("Make sure your method is named estimate!",
					"No Method called Estimate!");
		} catch (NoClassDefFoundError e) {
			dialogueBox("The class is formatted incorrectly!",
					"Class Format Error!");
		} catch (TooManyGuessesException e) {
			dialogueBox("You made too many guesses!", "Too Many Guesses!");
		} catch (SecurityException e) {
			dialogueBox(
					"You may have tried to access files outside of the project.",
					"Illegal Operation");
		} catch (IOException e) {
