...
public class GUITest  extends UISpecTestCase { 
    ...
    public void setUp() throws Exception {
        ...
        genPoly = mainPanel.getPanel("Side Panel").getPanel("GeneratePolygon Panel");
    }
    
    @Test
    public void testVisibilityOfEverythingMuahahahah() {
        assertTrue("Main panel: NOT visible!", mainPanel.isVisible());
        assertTrue("Console panel: NOT visible!", mainPanel.getPanel("Console Panel").isVisible());
        assertTrue("Polygon panel: NOT visible!", genPoly.isVisible());
        assertTrue("Score panel: NOT visible!", scorePanel.isVisible());
        assertTrue("Points table: NOT visible", window.getPanel("Points Panel").getPanel("Points Table").isVisible());
    }
	...
	final String incorrect = "Incorrect Area";
        ...
        genPoly.getTextBox(guess).setText("60");//This is too small a value - should be set up to 100
        genPoly.getTextBox(vert).setText("6");
        genPoly.getButton(poly).click();
        assertEquals(incorrect,"33.5814", getActArea().substring(22, 29));
        assertEquals("Invalid number of guesses", genPoly.getTextBox("Guesses Field").getText(), "100");
        
        
        genPoly.getTextBox(seed).setText("4517777");
        genPoly.getTextBox(guess).setText("9001");//This is too large a value - should be set back down to 300
        genPoly.getTextBox(vert).setText("35"); //This is too large a value - should be set back down to 20
        genPoly.getButton(poly).click();
        assertEquals(incorrect,"49.2843", getActArea().substring(22, 29));
        assertEquals("Invalid number of vertices", genPoly.getTextBox("Vertices field").getText(), "20");
        assertEquals("Invalid number of guesses", genPoly.getTextBox("Guesses Field").getText(), "300");
        

        genPoly.getTextBox(seed).setText("55116");
        genPoly.getTextBox(guess).setText("300");
        genPoly.getTextBox(vert).setText("2");//This is too small a value, both mathematically and for our program - should be set to 5.
        genPoly.getButton(poly).click();
        assertEquals(incorrect,"27.6805", getActArea().substring(22, 29));
        assertEquals("Invalid number of vertices", genPoly.getTextBox("Vertices field").getText(), "5");
        
        genPoly.getTextBox(seed).setText("9116518675309"); //This is an invalid seed.  This should be checked for and corrected, or an exception will be thrown.
        genPoly.getButton(poly).click();
    ...
    public void testRunAndScore() {
        ...
        assertEquals("Console not reset", "", mainPanel.getPanel("Console Panel").getTextBox("Console Output").getText());
    }
